...
usage: modprobe [-alqrv] [module [symbol=value ...]]

modprobe loads or removes kernel modules from the running system.
It reads modules.dep, modules.alias, and modules.symbols from the appropriate
/lib/modules/release directory to resolve module names and dependencies,
loading prerequisites first.

Without -r, modprobe loads the named module (and any required dependencies)
into the kernel.
*/
#include "arg.h"
#include "fs.h"
...
main(int argc, char *argv[])
{
	...
	: Per-module configuration snippets.

	`/lib/modules/`_release_`/modules.dep`
	: Module dependency map generated by `depmod(8)`.
	...
	1
	: Module not found or kernel rejected the operation.

	2
	: I/O error on stdout or stdin.

	## SEE ALSO

	insmod(8), rmmod(8), lsmod(8), depmod(8)
	*/
